...
class Intersection(wx.Frame):
  title="Narinthorn Intersection"	#Give a title name for user interface of Narinthorn Intersection
  def __init__(self):				#Initialize parameter of class Intersection
    ...
    self.PORT=2100																	#Set port as 2100
    self.run = True
    ...
    if self.run:
      if (len(self.data)>5):
        ...
        yek = [ (int(tmp[2])+int(tmp[3])+int(tmp[4])+int(tmp[5])+int(tmp[6])),		#to make each data from "tmp" becoming integer and store into "yek" each direction split by ',' for vehicles number
                (int(tmp[7])+int(tmp[8])+int(tmp[9])+int(tmp[10])),
                (int(tmp[11])+int(tmp[12])+int(tmp[13])+int(tmp[14])+int(tmp[15])),
                (int(tmp[16])+int(tmp[17])+int(tmp[18])+int(tmp[19])+int(tmp[20])) ]
        ...
        if ((len(self.data_x)%5)==4):
          ...
          axes1 = self.fig.add_subplot(121)											#add graph 1x2 store in graph 1  from Narathiwat, North Sathorn , South Sathorn
          ...
          axes1.set_ylabel('Vehicles', size=15)										#set label on y axes
          axes1.set_title('Narathiwat,North Sathorn,South Sathorn', size=10)		#set title of the graph
		  
          axes2 = self.fig.add_subplot(122)											#add graph 1x2 store in graph 2 from Narathiwat , South Sathorn
          ...
          axes2.set_title('Narathiwat,South Sathorn', size=10)						#set title of the graph
  ...
  def onChangeButton(self, event, button):
    ...
    if (button==0):											#Only North Sathorn having green signal
      self.plan.SetLabel('0 -- North Sathorn')
      self.g0 = 0
      self.g1 = 0
    if (button==1):											#Only Narathiwat (North) having green signal
      self.plan.SetLabel("1 -- Narathiwat(North)")
      self.g0 = 3
      self.g1 = 3
    if (button==2):											#Only Narathiwat(South) having green signal
      self.plan.SetLabel("2 -- Narathiwat(South)")
      self.g0 = 1
      self.g1 = 1
    if (button==3):											#Only South Sathorn having green signal
      self.plan.SetLabel('3 -- South Sathorn')
      self.g0 = 2
      self.g1 = 2
    if (button==4):											#Both North Sathorn and South Sathorn having green signal
      self.plan.SetLabel("4 -- North Sathorn - South Sathorn")
      self.g0 = 0
      self.g1 = 2
    self.resetGraph = True											#Reset the graph everytime phasing have been change
  ...
  def init_plot(self):											#Set figure of the plot by size and dot per inches
    #self.fig = Figure((4.5, 2.55), dpi=100)
  ...
  def buildUI(self):											#building user interface for plotting and trafficlight button
    ...
    self.button = [ wxArt.ToggleButton(self.bg, -1,'pic/narinthorn/narinthorn_1.png','pic/narinthorn/narinthorn_1_on.png',(50,720),(125, 125)),	#Using toggle button from wxArt and import both "on" and "off" picture
                    wxArt.ToggleButton(self.bg, -1,'pic/narinthorn/narinthorn_2.png','pic/narinthorn/narinthorn_2_on.png',(190,720),(125, 125)),
                    wxArt.ToggleButton(self.bg, -1,'pic/narinthorn/narinthorn_3.png','pic/narinthorn/narinthorn_3_on.png',(330,720),(125, 125)),
                    wxArt.ToggleButton(self.bg, -1,'pic/narinthorn/narinthorn_4.png','pic/narinthorn/narinthorn_4_on.png',(470,720),(125, 125)),
                    wxArt.ToggleButton(self.bg, -1,'pic/narinthorn/narinthorn_5.png','pic/narinthorn/narinthorn_5_on.png',(50,880),(125, 125))]
