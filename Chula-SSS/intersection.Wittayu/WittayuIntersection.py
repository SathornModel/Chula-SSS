...
class Intersection(wx.Frame):
  title="Wittayu Intersection"	#Give a title name for user interface of Wittayu Intersection
  def __init__(self):				#Initialize parameter of class Intersection
    ...
    self.PORT=13400																	#Set port as 13400
    self.run = True
  ...
  def updateTimer(self,event):
    if self.run:
      if (len(self.data)>5):
        ...
        yek = [ (int(tmp[2])+int(tmp[3])+int(tmp[4])+int(tmp[5])+int(tmp[6])),		#to make each data from "tmp" becoming integer and store into "yek" each direction split by ',' for vehicles number
                (int(tmp[7])+int(tmp[8])+int(tmp[9])),								#each tmp represent one induction loop sensor
                (int(tmp[10])+int(tmp[11])+int(tmp[12])+int(tmp[13])+int(tmp[14])),
        ...
        if ((len(self.data_x)%5)==4):
          self.fig.clf()
          axes1 = self.fig.add_subplot(121)											#add graph 1x2 store in graph 1  from Rama IV(Out),Rama IV(In),North Sathorn,Wittayu
          ...
          axes1.set_title('Rama IV(Out),Rama IV(In),North Sathorn,Wittayu', size=10)#set title of the graph
		  
          axes2 = self.fig.add_subplot(122)											#add graph 1x2 store in graph 2 from Rama IV(Out),Rama IV(In),Wittayu
          ...
          axes2.set_title('Rama IV(Out),Rama IV(In),Wittayu', size=10)				#set title of the graph
  ...
  def onChangeButton(self, event, button):
    ...
    if (button==0):											#Only Wittayu having green signal
      self.plan.SetLabel('0 -- Wittayu')
      self.g0 = 2
      self.g1 = 2
    if (button==1):											#Only Rama IV(In) having green signal
      self.plan.SetLabel("1 -- Rama IV(In)")
      self.g0 = 1
      self.g1 = 1
    if (button==2):											#Both Rama IV(In) and Rama IV(Out) having green signal
      self.plan.SetLabel("2 -- Rama IV(In) - Rama IV(Out)")
      self.g0 = 3
      self.g1 = 1
    if (button==3):											#Both Rama IV(Out) and Rama IV(In) having green signal
      self.plan.SetLabel('3 -- Rama IV(Out) - Rama IV(In)')
      self.g0 = 3
      self.g1 = 1
    if (button==4):											#Both North Sathorn and Wittayu having green signal
      self.plan.SetLabel("4 -- North Sathorn - Wittayu")
      self.g0 = 0
      self.g1 = 2
    if (button==5):											#Both North Sathorn and Wittayu having green signal
      self.plan.SetLabel("5 -- North Sathorn - Wittayu")
      self.g0 = 0
      self.g1 = 2
    if (button==6):											#Only North Sathorn having green signal
      self.plan.SetLabel("6 -- North Sathorn")
      self.g0 = 0
      self.g1 = 0
    if (button==7):											#Only Rama IV(Out) having green signal
      self.plan.SetLabel("7 -- Rama IV(Out)")
      self.g0 = 3
      self.g1 = 3
    self.resetGraph = True
  ...
  def init_plot(self):											#Set figure of the plot by size and dot per inches
    self.fig = Figure((9.5, 5), dpi=100)
  ...
  def buildUI(self):											#building user interface for plotting and trafficlight button
    ...
    self.button = [ wxArt.ToggleButton(self.bg, -1,'pic/wittayu/wittayu_1.png','pic/wittayu/wittayu_1_on.png',(50,720),(125, 125)),#Using toggle button from wxArt and import both "on" and "off" picture
                    wxArt.ToggleButton(self.bg, -1,'pic/wittayu/wittayu_2.png','pic/wittayu/wittayu_2_on.png',(190,720),(125, 125)),
                    wxArt.ToggleButton(self.bg, -1,'pic/wittayu/wittayu_3.png','pic/wittayu/wittayu_3_on.png',(330,720),(125, 125)),
                    wxArt.ToggleButton(self.bg, -1,'pic/wittayu/wittayu_4.png','pic/wittayu/wittayu_4_on.png',(470,720),(125, 125)),
                    wxArt.ToggleButton(self.bg, -1,'pic/wittayu/wittayu_5.png','pic/wittayu/wittayu_5_on.png',(50,880),(125, 125)),
                    wxArt.ToggleButton(self.bg, -1,'pic/wittayu/wittayu_6.png','pic/wittayu/wittayu_6_on.png',(190,880),(125, 125)),
                    wxArt.ToggleButton(self.bg, -1,'pic/wittayu/wittayu_7.png','pic/wittayu/wittayu_7_on.png',(330,880),(125, 125)),
                    wxArt.ToggleButton(self.bg, -1,'pic/wittayu/wittayu_8.png','pic/wittayu/wittayu_8_on.png',(470,880),(125, 125))]
